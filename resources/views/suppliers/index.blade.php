...
@section('title', 'Suppliers - YEGNA TRADING PLC')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <tr class="{{ $loop->index % 2 === 1 ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-blue-50/30 transition-colors border-b border-gray-100 last:border-0">
                        ...
                        <td class="px-4 py-3.5 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-1">
                                <x-table-action :href="route('suppliers.edit', $supplier)" icon="pencil" variant="primary" tooltip="Edit" />
                                <x-table-action :href="route('suppliers.destroy', $supplier)" icon="trash" variant="danger" method="DELETE" confirm confirm-message="Delete this supplier?" tooltip="Delete" />
                            </div>
                        </td>
                    </tr>
                @empty
    ...
    <div class="px-4 py-3 border-t border-gray-200 bg-gray-50/50">
        <p class="text-xs text-gray-500">
            Showing <span class="font-medium text-gray-700">{{ $suppliers->firstItem() ?? 0 }}</span>
            to <span class="font-medium text-gray-700">{{ $suppliers->lastItem() ?? 0 }}</span>
            of <span class="font-medium text-gray-700">{{ $suppliers->total() }}</span> suppliers
        </p>
    </div>
</div>

<x-pagination :paginator="$suppliers" label="suppliers" />
@endsection
