...
@section('title', 'Edit Supplier - YEGNA TRADING PLC')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $supplier->name) }}" required
                           ...
                           placeholder="e.g. Ethio Steel Import">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="company_name" id="company_name" value="{{ old('company_name', $supplier->company_name) }}"
                           ...
                           placeholder="e.g. Addis Ababa Construction Materials PLC">
                    @error('company_name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                              ...
                              placeholder="Optional address for this supplier">{{ old('address', $supplier->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border-t border-gray-200 pt-5">
                    <p class="text-sm font-semibold text-gray-700 mb-3">Default Rebar Specifications</p>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div>
                            <label for="default_type" class="block text-sm font-medium text-gray-700 mb-1">Rebar Type</label>
                            <input type="text" name="default_type" id="default_type" value="{{ old('default_type', $supplier->default_type) }}"
                                   class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('default_type') border-red-500 @enderror"
                                   placeholder="e.g. Gr60">
                            @error('default_type')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="default_diameter" class="block text-sm font-medium text-gray-700 mb-1">Diameter</label>
                            <input type="text" name="default_diameter" id="default_diameter" value="{{ old('default_diameter', $supplier->default_diameter) }}"
                                   class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('default_diameter') border-red-500 @enderror"
                                   placeholder="e.g. 12mm">
                            @error('default_diameter')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="default_size" class="block text-sm font-medium text-gray-700 mb-1">Size</label>
                            <input type="text" name="default_size" id="default_size" value="{{ old('default_size', $supplier->default_size) }}"
                                   class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('default_size') border-red-500 @enderror"
                                   placeholder="e.g. 12m">
                            @error('default_size')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <p class="text-xs text-gray-500 mt-2">These defaults auto-fill when selecting this supplier for purchases.</p>
                </div>
